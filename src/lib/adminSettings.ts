...
export type AdminSettings = {
  /**
   * Whether the public pages print what a course of treatment costs.
   *
   * Not a purchase switch: since the consultation-first cutover nobody can
   * buy a programme directly, so this only decides whether a visitor is
   * shown the price of something their therapist may later recommend.
   * Reads `show_programme_prices`, falling back to the older
   * `session_packages_visible` it was seeded from.
   */
  showProgrammePrices: boolean;
  ...
  /** Whether the detector sweep runs at all. */
  riskSignalsEnabled: boolean;
  /** Assign a paid session automatically when exactly one therapist is
   *  unambiguously free for it, rather than leaving it in the admin queue.
   *  See src/lib/autoAssignTherapist.ts. */
  autoAssignTherapistEnabled: boolean;
  sessionTimeoutMinutes: number;
  /** Seconds the post-logout banner stays up. 0 = until dismissed. */
  farewellBannerSeconds: number;
  googleMeetEnabled: boolean;
  /** Whether each new session's Meet space is switched to OPEN access, so
   *  neither the patient nor the therapist has to be admitted from a waiting
   *  room. See src/lib/googleMeetSpace.ts. */
  meetOpenAccessEnabled: boolean;
...
export const DEFAULT_ADMIN_SETTINGS: AdminSettings = {
  ...
  // Off for one release: it changes what happens to a booking after money
  // has moved, so the previous behaviour stays one click away.
  autoAssignTherapistEnabled: false,
...
type SiteSettingsRow = {
  ...
  risk_signals_enabled?: boolean | null;
  auto_assign_therapist_enabled?: boolean | null;
  session_timeout_minutes?: number | null;
  google_meet_enabled?: boolean | null;
  meet_open_access_enabled?: boolean | null;
  ...
  return {
    ...
    carePlanDefaultExpiryDays:
      typeof row?.care_plan_default_expiry_days === "number"
        ...
        : DEFAULT_ADMIN_SETTINGS.carePlanDefaultExpiryDays,
    carePlanMaxFrequencyPerWeek:
    ...
    riskSignalsEnabled:
      typeof row?.risk_signals_enabled === "boolean"
        ? row.risk_signals_enabled
        : DEFAULT_ADMIN_SETTINGS.riskSignalsEnabled,
    autoAssignTherapistEnabled:
      typeof row?.auto_assign_therapist_enabled === "boolean"
        ? row.auto_assign_therapist_enabled
        : DEFAULT_ADMIN_SETTINGS.autoAssignTherapistEnabled,
    ...
    meetOpenAccessEnabled:
      typeof row?.meet_open_access_enabled === "boolean"
        ? row.meet_open_access_enabled
        : DEFAULT_ADMIN_SETTINGS.meetOpenAccessEnabled,
    joinWindowMinutes:
