...
export default function PackagePurchaseDetailModal({
  ...
  // Starts true (rather than being flipped true synchronously inside the
  // effect below, which React's rules disallow) -- the caller mounts this
  // component fresh with key={purchaseId} per open, so "true at mount" is
  // exactly the right initial state for every purchase this ever opens for.
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  // Lazy initializer -- read once at mount, not on every render. Same
  // pattern as ProfileSessionList/BookingWizard elsewhere in this codebase.
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/admin/package-purchase-detail?id=${purchaseId}`)
      .then((res) => res.json())
      .then((json) => {
        if (cancelled) return;
        if (json.error) {
  ...
  async function refetch() {
    const res = await fetch(`/api/admin/package-purchase-detail?id=${purchaseId}`);
  ...
  async function handleExtendExpiry() {
    ...
    await runAction("/api/admin/extend-package-expiry", { purchaseId, newExpiresAt, reason });
  }

  async function handleRestore(appointmentId: string) {
    if (!(await confirm("Give this session back to the patient's balance?"))) return;
    await runAction("/api/admin/restore-package-session", { purchaseId, appointmentId });
  }

  async function handleRefund() {
    if (
      !(await confirm(
        "Refund every unused/scheduled session on this package and close it out? Scheduled sessions will be cancelled."
      ))
    )
      return;
    const reason = await prompt("Reason for the refund (shown in the timeline):", "");
    if (reason === null) return;
    await runAction("/api/admin/refund-package", { purchaseId, reason });
  }

  const counts = data
    ? computePackageCounts({
        sessionCount: data.purchase.session_count,
        sessionsUsed: data.purchase.sessions_used,
        completedCount: data.completed.length,
        scheduledCount: data.upcoming.length,
      })
    : null;
  const daysLeft = data ? daysUntilExpiry(data.purchase.expires_at, now) : null;

  return (
    <Modal
      title={data ? data.purchase.packageTitle : "Package"}
      ...
      {data && counts && (
        <div className="space-y-6 text-xs">
          ...
          {data.restorable.length > 0 && (
            <section>
              <h4 className="font-bold text-slate-800 mb-2">Forfeited sessions</h4>
              <ul className="space-y-1.5">
