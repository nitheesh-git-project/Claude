...
export default function ResetTherapistPasswordButton({
  therapistId,
  currentPassword,
  ...
  async function handleReset() {
    if (
      !window.confirm(
        "This will invalidate the therapist's current password immediately. Continue?"
      )
    ) {
      return;
    }
    setLoading(true);
    setError(null);
    const res = await fetch("/api/admin/reset-therapist-password", {
      method: "POST",
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-xl p-3 text-xs space-y-1.5">
        <p className="font-bold text-teal-900">
          ...
          <span className="font-normal text-teal-700">
            (visible here until the therapist sets their own)
          </span>
          :
        </p>
        <p>
