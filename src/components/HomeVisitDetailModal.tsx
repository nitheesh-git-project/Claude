...
/**
 * Home-visit twin of PackageDetailModal -- shared by the patient dashboard's
 * home-visit package widget and (once wired) the therapist Programme
 * Patients list. Same accessible-dialog implementation as that component
 * and TeamTherapistPopup.
 */
export default function HomeVisitDetailModal({
  purchaseId,
  onClose,
}: {
  purchaseId: string;
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/home-visit/purchase-detail?id=${purchaseId}`)
      .then((res) => res.json())
      .then((json) => {
        if (cancelled) return;
  ...
  const counts = data
    ? computeHomeVisitCounts({
        visitCount: data.purchase.visitCount,
        visitsUsed: data.purchase.visitsUsed,
        completedCount: data.completed.length,
        scheduledCount: data.upcoming.length,
      })
    : null;
  const daysLeft = data ? daysUntilHomeVisitExpiry(data.purchase.expiresAt, now) : null;
      ...
      >
        ...
        >
          <div className="relative overflow-hidden rounded-t-3xl bg-gradient-to-br from-teal-800 to-emerald-700 px-6 pb-7 pt-6 sm:px-8">
            ...
            <h3 id="home-visit-modal-title" className="font-display pr-10 text-xl font-extrabold text-white sm:text-2xl">
              {data?.purchase.packageTitle ?? "Home Visit Package"}
            </h3>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs text-slate-600">
                  <p>
                    ...
                    {data.purchase.paymentMode === "cash_on_visit" ? "Cash on visit" : "Prepaid"}
                  </p>
                ...
                {counts.pending > 0 && (
                  <p className="text-xs text-slate-400">{counts.pending} visit(s) not yet scheduled.</p>
                )}
